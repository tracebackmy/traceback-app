...
export default function ClaimItemPage() {
  ...
  const [ticketId, setTicketId] = useState<string | null>(null); // New: Track associated ticket
    ...
    try {
      ...
      if (user) {
        ...
        if (!claimSnapshot.empty) {
          ...
          // 3. New: Fetch Associated Ticket
          // We look for a ticket where 'relatedId' matches this claim's ID
          const ticketQ = query(
            collection(db, 'tickets'),
            where('relatedId', '==', claimData.id),
            where('contextType', '==', 'claim_inquiry')
          );
          const ticketSnapshot = await getDocs(ticketQ);
          if (!ticketSnapshot.empty) {
            setTicketId(ticketSnapshot.docs[0].id);
          }
        } else {
          setUserClaim(null);
          setTicketId(null);
      ...
      <div className="min-h-screen bg-gray-50 p-6 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-xl shadow-sm border border-blue-200 p-8 text-center">
            ...
            <div className="flex flex-col gap-3">
              ...
              {/* New Message Admin Button */}
              {ticketId && (
                <button
                  onClick={() => {
                    // For now, redirect to dashboard with param, or just notify user
                    // In Phase 4 we will auto-open this ticket
                    alert('Support ticket is active. Please check your chat bubble.');
                  }}
                  className="w-full bg-white text-blue-600 font-medium py-3 border border-blue-200 rounded-lg hover:bg-blue-50 transition-colors flex items-center justify-center gap-2"
                >
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8 text-center">
          ...
          <p className="mb-6 text-gray-600">The admin will review your request shortly. A support ticket has been created for you.</p>
